...
ggplot(EEG_v3, aes(x = cond, y = Value, color = as.factor(trial_type), group = trial_type)) +
...
EEG_v3$Type <- with(EEG_v3, ifelse(cond == -1, "Oddball", "Reversal"))

ggplot(EEG_v3, aes(x = interaction(cond, trial_type), y = Value, color = Type)) +
  geom_point(alpha = .1) +
