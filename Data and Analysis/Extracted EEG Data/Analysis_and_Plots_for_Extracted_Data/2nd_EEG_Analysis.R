...
ggplot(EEG_v2, aes(x = cond, y = Value, color = as.factor(trial_type), group = trial_type)) +
...
EEG_v2$Type <- with(EEG_v2, ifelse(cond == -1, "Oddball", "Reversal"))

ggplot(EEG_v2, aes(x = interaction(cond, trial_type), y = Value, color = Type)) +
  geom_point(alpha = .1) +
